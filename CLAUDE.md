...
**Map 2 Visual Richness, Increment 3 (final): organic decoration + ambient animation — the third
and last of the small, independently-confirmed increments (after Increment 1's gradient and
Increment 2's icons, both left completely untouched here). Same hard boundary as every prior
pass: positioning, lane-assignment/reuse logic, right-angle connector routing, Overview lock/
unlock logic, and the Increment 1/2 work are all untouched — `npm run verify:spacing` confirmed
20/20, byte-for-byte identical, both before and after this change.**
- **Task 1 — small leaf/sprig decorations "growing off" connectors.** `leafSpot(x1, y1, x2, y2,
  side)` (a new pure helper, Roadmap.jsx) derives a leaf's position/angle purely from a segment's
  own TWO ALREADY-COMPUTED endpoints — the segment's own midpoint, nudged a fixed 12px to one side
  along its perpendicular, plus the segment's own real angle so the `Leaf` icon can be rotated to
  sit naturally along whichever direction that particular segment runs (vertical for every
  within-lane step-to-step connector; always horizontal for a lane's own jog out from the spine,
  per the "Fix: Starter Task..." section's own established fact that the jog is now a pure
  horizontal reach) — this is purely derived decoration, never a reason for any segment's own `d`
  to change. Only rendered when a segment's own real length exceeds `MIN_LEAF_SEGMENT` (34px), so
  a floored/near-zero-length gap never crowds a leaf on top of its own two endpoints. Leaves use a
  fixed `var(--bloom-green)` (not the lane's own track color) — a literal leaf reading as anything
  other than natural green would look wrong regardless of which interest track its chain happens
  to represent. Positioning/orientation is a plain, static SVG `transform="rotate(...)"`
  ATTRIBUTE; the "growing" life (Task 3) is a CSS animation on `opacity` ONLY (`lane-leaf-sway`),
  deliberately never `transform` — this codebase has hit the "a CSS transform silently REPLACES
  rather than composes with an SVG presentation-attribute transform" landmine several times
  already (WelcomeScreen's markers, this same component's own node transforms, MascotIcon's leaf
  sway, the hub tile pop-in) — animating a completely different CSS property here sidesteps that
  landmine entirely rather than needing a second nested wrapper group just to avoid it.
- **Task 2 — ambient background particles.** `ambientParticles` (a `useMemo`, keyed on
  `roadmap.canvasWidth`/`canvasHeight`) is a fixed-count (22) scatter using `seededRandom` (a tiny
  deterministic sine-based pseudo-random function, NOT `Math.random()`) so the scatter stays
  ...
  drive it change" discipline `flatPlanItems` already established. Capped at a fixed count
  regardless of canvas height, so a dense multi-year plan's much taller canvas never means
  proportionally more clutter. About 28% render as a small `Sparkle` glyph (own independent
...
- **Task 3 — animation throughout, layered strictly on top of existing ring/connector styling.**
  `.node-halo` (the pre-existing soft static circle behind every ring) gained a real
  `filter: blur(5px) drop-shadow(0 0 5px var(--halo-color, transparent))` — `--halo-color` is a
  plain inline CSS custom property set per-circle in Roadmap.jsx (`style={{ '--halo-color':
  ...
  / 0.16 required/clusters) is completely unaffected; this only adds softness/bloom on top. A
  gentle "breathing" pulse (`node-halo-breathe`) is added via a NEW `.node-halo-active` class,
  applied only to an INCOMPLETE, UNLOCKED node's halo (`!done`, checked in the exact same branch
  of Roadmap.jsx's ternary that already guarantees `!locked` — a locked node's own halo never gets
  this at all, rendered in its own dimmed 0.5-opacity group with no animation). Deliberately
  animates only `transform: scale(...)` and `filter` (blur/drop-shadow intensity) — NEVER the
  `opacity` CSS property directly, which would silently override (and so discard) each node's own
  per-type resting alpha set via the SVG `opacity` ATTRIBUTE, since a CSS property of the same
  name always wins over a presentation attribute. Today's own marker never carries `.node-halo` at
  all (it has its own separate, pre-existing `.today-pulse`/`.today-glow` pair) — it stays the one
  unambiguously distinct, most-celebratory marker on the canvas regardless of this new universal
  breathing glow on everything else.
- **Every one of the 3 new animation systems (halo breathe, leaf sway, particle drift/sparkle) was
  added to the existing `prefers-reduced-motion` disable block** alongside this component's other
  animations — confirmed directly (not assumed) via a real `getComputedStyle().animationName`
  check under both a forced `reducedMotion: 'reduce'` browser context (all 3 resolve to `none`,
  with every decorative element still genuinely PRESENT, just static — 12 halos / 8 leaves / 22
  particles all still rendered) and normal motion (all 3 resolve to their real keyframe names).
- **A real, deliberate lesson carried over from the earlier reverted all-at-once attempt**: this
  time, before any browser-based verification, every stale/duplicate Vite dev server process for
  this project was killed and `node_modules/.vite` was cleared, then a single fresh server was
  started — closing off the exact class of false alarm ("glow moving erratically") that turned
  out, in that earlier session, to be a stale dev-server/cache artifact rather than a real code
  bug. DOM-level checks (element counts, computed `animationName`, gradient stop offsets, icon
  class names, curve-command scans) were treated as the authoritative verification throughout —
  screenshots were used only as a supplementary sanity check, exactly the discipline that already
  caught a stray Playwright zoom-targeting artifact during Increment 1's own verification.
- Verified with a dedicated DOM scan (normal + reduced motion, side by side) across a realistic
  dense plan (2 opportunity chains, a started Build-Your-Own project, a custom task): 8 real
  leaves and 22 real particles render in both motion modes; all 3 new animations are active under
  normal motion and correctly `none` under reduced motion; Increment 1's gradient is completely
  unaffected (3 spine stops at the same `0`/`0.5`/`1` offsets, per-lane gradients still present);
  Increment 2's icon set is completely unaffected (`sparkle`/`footprints`/`map-pin`/`list-checks`/
  `pencil`/`circle-check` all still resolve correctly); and 0 of the real connector paths contain
  any curve command. A tight close-up screenshot confirms the glow reads as genuinely soft/subtle
  (not garish) and the ambient particles are faint background texture, not visual clutter — the
  explicit "calm and rich, not busy or chaotic" bar this increment was held to. `npm run build`/
  `npm run lint`/`npm run verify:spacing` (20/20) all stay clean.
