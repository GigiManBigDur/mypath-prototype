...
**Map 2 Visual Richness Pass — a pure styling/decoration overlay on top of the Fixed-Lane +
Right-Angle-Connector architecture, matching a reference image's own colorful/glowing/organic
character while explicitly NOT adopting its curved connector shapes. Same hard boundary as every
prior pass on this component: `roadmapLayout.js`'s date-to-y math, `roadmapGenerator.js`'s
lane-assignment/lock/date-derivation logic, and the connector routing itself are all untouched —
`npm run verify:spacing` was re-confirmed at 20/20 after EACH of the 3 build stages below, not just
once at the end, and a dedicated DOM check confirms every real connector path (identified by its
own `stroke="url(#rm-spine-gradient)"`/`url(#rm-lane-gradient-*)` reference, not just any `<path>`
— lucide icon glyphs are also `<path>` elements and do legitimately contain curve commands) still
resolves to nothing but `M`/`L` segments across a dense 3-chain + Build-Your-Own-project plan.**
- **Stage 1 — a real 3-stop "journey" gradient** replaces the old flat 2-stop teal fade on the
  spine's own solid connector: `<linearGradient id="rm-spine-gradient" gradientUnits=
  "userSpaceOnUse">` anchored to real canvas y-coordinates (today's own y at the bottom, `0` at the
  top — NOT each segment's own bounding box, which would restart the gradient per-segment instead
  of reading as one continuous sweep) with stops `--bloom-accent` (cool green, near "You are
  here") → `--bloom-purple` (middle) → `--bloom-yellow` (warm gold, furthest along) — a pure paint-
  server definition, changing only what PAINTS the already-correct `d`, never the `d`/coordinates
  themselves. **Every chain's own lane connector gets its own per-lane
  `<linearGradient id="rm-lane-gradient-${n.id}">`**, built from that chain's own real, already-
  meaningful `configFor(n).color` (track-colored for a real opportunity, bloom-purple for a
  project, generic fallback otherwise) — deliberately NOT swapped to the same global teal/purple/
  gold journey hue the spine uses, since that color is real per-track interest-area coding (see
  Task 2's own comment on `configFor`) and a generic position-based gradient would blur that
  meaning away; instead the gradient varies OPACITY (0.55 at the anchor end sweeping to 1 at the
  chain's own far end) so the connector gets a genuine "shifts along the timeline" gradient sweep
  while its hue stays fully intact. Both lane's stroke attributes were repointed from a flat
  `branchColor` variable (removed as dead code) to `url(#rm-lane-gradient-${n.id})`.
- **Stage 2 — meaningfully varied, purposeful icons, not a generic recolor.** Two of this file's
  own existing generic `Circle` icons were swapped: `CORE_TYPE_CONFIG.procedure` and
  `BRANCH_STEP_CONFIG` (a chain's own generic middle/non-last step) both moved to `Footprints` —
  deliberately avoiding the "empty circle sitting inside an empty ring" double-meaning `Circle`
  had, since a plain circle is also this app's own universal "not done" checkbox glyph everywhere
  else, which read as visually confusing/redundant specifically on an incomplete node. Every OTHER
  `CORE_TYPE_CONFIG`/`OPPORTUNITY_CONFIG`/`CUSTOM_CONFIG`/`PROJECT_CONFIG`/`CLUSTER_CONFIG` entry
  already carried a real, distinct, purposeful icon from earlier passes (`MapPin` for milestones,
  `Lock` for a locked Overview, `Star`/`Flag` for major/final goals, `Rocket` for the plan's own
  achievement milestone, `Bell` for a due-today collision, `CreditCard`/`ListChecks`/etc. for their
  own specific task types) — confirmed directly via a real DOM scan (not assumed) across a dense
  test plan: 34 rendered nodes resolved to 6 genuinely distinct icon glyphs (`footprints`, `flag`,
  `map-pin`, `list-checks`, `star`, `credit-card`), not one repeated generic shape.
- **Stage 3 — glow, organic decorations, and ambient animation, all layered strictly ON TOP of
  existing ring/connector styling, never replacing it.**
  - **Glow (Task 2)**: `.node-halo` (the pre-existing soft static circle behind every ring) gained
    a real `filter: blur(5px) drop-shadow(0 0 5px var(--halo-color, transparent))` — `--halo-color`
    is a plain inline CSS custom property set per-circle in Roadmap.jsx (`style={{ '--halo-color':
    cfg.color }}`), since CSS has no way to read an SVG element's own `fill` attribute back into a
    `var()`. This never touches the halo's own `fill`/`opacity` SVG attributes (still set directly
    per node type, unchanged) — every node's carefully-tuned resting alpha (0.1 locked / 0.14 most
    / 0.16 required/clusters) is completely unaffected; this only adds softness/bloom on top.
  - **A gentle "breathing" pulse (`node-halo-breathe`, Task 5)** is added via a NEW `.node-halo-
    active` class, applied only to an INCOMPLETE, UNLOCKED node's halo (`!done`, checked in the
    exact same branch of Roadmap.jsx's ternary that already guarantees `!locked` — a locked node's
    own halo never gets this at all, rendered in its own dimmed 0.5-opacity group with no
    animation). Deliberately animates only `transform: scale(...)` and `filter` (blur/drop-shadow
    intensity) — NEVER the `opacity` CSS property directly, which would silently override (and so
    discard) each node's own per-type resting alpha set via the SVG `opacity` ATTRIBUTE, since a
    CSS property of the same name always wins over a presentation attribute. Today's own marker
    never carries `.node-halo` at all (it has its own separate, pre-existing `.today-pulse`/
    `.today-glow` pair) — it stays the one unambiguously distinct, most-celebratory marker on the
    canvas regardless of this new universal breathing glow on everything else.
  - **Organic decorations (Task 4) — small leaf/sprig glyphs "growing off" connectors at
    intervals.** `leafSpot(x1, y1, x2, y2, side)` (a new pure helper, Roadmap.jsx) derives a leaf's
    position/angle purely from a segment's own TWO ALREADY-COMPUTED endpoints — the segment's own
    midpoint, nudged a fixed 12px to one side along its perpendicular, plus the segment's own real
    angle so the `Leaf` icon can be rotated to sit naturally along whichever direction that
    particular segment runs (vertical for every within-lane step-to-step connector; always
    horizontal for a lane's own jog out from the spine, per the "Fix: Starter Task..." section's
    own established fact that the jog is now a pure horizontal reach) — this is purely derived
    decoration, never a reason for any segment's own `d` to change. Only rendered when a segment's
    own real length exceeds `MIN_LEAF_SEGMENT` (34px), so a floored/near-zero-length gap never
    crowds a leaf on top of its own two endpoints. Leaves use a fixed `var(--bloom-green)` (not the
    lane's own track color) — a literal leaf reading as anything other than a natural green would
    look wrong regardless of which interest track its chain happens to represent, so this is a
    deliberate universal exception to the "everything reads the resolved track color" convention
    every other decoration in this pass follows. **Positioning/orientation is a plain, static SVG
    `transform="rotate(...)"` ATTRIBUTE; the Task-5 "growing" life is a CSS animation on `opacity`
    ONLY (`lane-leaf-sway`), deliberately never `transform`** — this codebase has hit the "a CSS
    `transform` silently REPLACES rather than composes with an SVG presentation-attribute
    `transform`" landmine several times already (WelcomeScreen's markers, this same component's own
    node transforms, MascotIcon's leaf sway, the hub tile pop-in) — animating a completely
    different CSS property here sidesteps that landmine entirely rather than needing a second
    nested wrapper group just to avoid it.
  - **Ambient background particles (Task 4/5)** — `ambientParticles` (a `useMemo`, keyed on
    `roadmap.canvasWidth`/`canvasHeight`) is a fixed-count (22) scatter using `seededRandom` (a
    tiny deterministic sine-based pseudo-random function, NOT `Math.random()`) so the scatter stays
    STABLE across ordinary re-renders (e.g. marking a node complete) rather than visibly reshuffling
    on every unrelated state change — the same "recompute only when the real inputs that should
    drive it change" discipline `flatPlanItems`/`digestGroups` above already established. Capped at
    a fixed count regardless of canvas height, so a dense multi-year plan's much taller canvas never
    means proportionally more clutter. About 28% render as a small `Sparkle` glyph (own independent
    rotation-plus-drift keyframe), the rest as plain small circles (`var(--bloom-purple)`,
    translateY drift + opacity fade) — both purely decorative (`pointer-events: none`, inherited
    from the parent `.ambient-particles` group, itself rendered FIRST/behind the axis/spine/every
    real node so it never competes with or obscures real content, matching the axis ticks' own
    "background reference, drawn first" precedent).
  - **A subtle shimmer on the connector gradient itself (Task 5)** — each of the SPINE gradient's
    3 `<stop>` elements gets a `rm-gradient-shimmer-stop` class animating `stop-opacity` gently (1
    ↔ 0.78), staggered per-stop via inline `animationDelay` so it reads as a shimmer gently
    traveling the spine's length rather than the whole line pulsing in lockstep. **Deliberately NOT
    applied to the lane gradients' own stops** — those intentionally hold two DIFFERENT base
    opacities (0.55/1, Stage 1's own "sweep along its own length") and an absolute-value shimmer
    keyframe would override/flatten that intentional difference at rest, the exact same "a CSS
    animation on a property fully overrides it, discarding whatever base value was there" trap the
    halo's own breathing animation above was already built to avoid for the identical reason.
  - **Every one of the 4 new animation systems (halo breathe, leaf sway, particle drift/sparkle,
    gradient shimmer) was added to the existing `prefers-reduced-motion` disable block** alongside
    this component's other animations — confirmed directly (not assumed) via a real
    `getComputedStyle().animationName` check under a forced `reducedMotion: 'reduce'` browser
    context: all 4 resolve to `none` there (with every decorative element still genuinely PRESENT,
    just static — 40 halos / 13 leaves / 22 particles all still rendered) and to their real
    keyframe names under normal motion.
- **Explicit staged build/verify process, exactly as requested**: color gradient + glow were built
  and screenshotted first, `npm run verify:spacing` re-confirmed 20/20 before touching icons;
  icons were redesigned second, re-verified again (20/20, plus a real DOM icon-class scan); organic
  decorations + ambient animation were added last, re-verified a third time (20/20) — never bundled
  into one combined change. A final pass re-ran `npm run build`/`npm run lint`/`npm run
  verify:spacing` together, plus a dedicated connector-path scan (`stroke` referencing this file's
  own gradient defs specifically, to exclude lucide icon glyphs — which are ALSO `<path>` elements
  and DO legitimately contain curve commands, that's not a bug) confirming 0 of 21 real connector
  paths across a dense 3-opportunity + Build-Your-Own-project test plan contain any curve command
  (`C`/`S`/`Q`/`T`/`A`, upper or lower case) — every connector is still exactly what the Fixed-Lane
  architecture already builds: straight horizontal jogs and straight vertical lane segments, never
  a curve. `npm run build`/`npm run lint` both stay clean throughout.
...
- Map 2 Visual Richness Pass: after touching ANY of `Roadmap.jsx`/`global.css` for this component,
  run `npm run verify:spacing` — it must stay 20/20 byte-for-byte, since this whole pass is styling
  only. Confirm the gradient defs directly via DOM inspection (`#rm-spine-gradient`'s 3 real stops,
  and one `#rm-lane-gradient-<id>` per real chain, each built from that chain's own real
  `configFor(n).color`) rather than trusting a screenshot alone — a flat color swap and a genuine
  gradient can look identical at a glance depending on zoom level. To confirm icon variety, scan
  every `.node-icon-pop` element's own `class` attribute for its `lucide-<name>` marker and count
  distinct names across a dense multi-opportunity test plan — this caught the fact that 6 genuinely
  different icons render (not a screenshot-only "looks varied" impression). **To verify the "no
  curves anywhere" constraint, don't check every `<path>` in the SVG** — lucide icon glyphs are
  ALSO `<path>` elements and legitimately contain real curve commands (`C`/`Q`/`S`/`T`/`A`); filter
  specifically to paths whose own `stroke` attribute references this component's gradient defs
  (`stroke^="url(#rm-spine-gradient"` or `stroke^="url(#rm-lane-gradient-"`), then confirm zero of
  those match a curve-command regex. To verify the glow/leaf/particle decorations render correctly
  (they're often imperceptible at the roadmap's default zoomed-out fit), locate a real node via its
  `data-node-id` attribute, read its `boundingClientRect()`, and drive real `page.mouse.wheel()`
  zoom-in events centered on that point before screenshotting a tight `clip` region around it —
  don't rely on a single full-canvas screenshot to judge decoration quality. Confirm both directions
  of `prefers-reduced-motion`: under a forced `reducedMotion: 'reduce'` context, every one of the 4
  new animation classes (`.node-halo-active`, `.lane-leaf`, `.ambient-particle`, `.rm-gradient-
  shimmer-stop`) should resolve `getComputedStyle(el).animationName` to `none` while the decorative
  elements themselves stay genuinely present (not removed) at their static rest state; under normal
  motion, all 4 should resolve to their real keyframe names. If you ever add a NEW absolute-value
  CSS animation to a property whose element already carries a per-node/per-type BASE value for that
  same property (an SVG `opacity`/`stop-opacity` attribute, a per-track `fill`), check whether every
  element sharing that animation class actually shares the same base value first — this pass hit
  that exact trap once already (the lane gradient's own two intentionally-different stop
  opacities) and deliberately left that one connector type unshimmered rather than flattening it.
